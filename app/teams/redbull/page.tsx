...
export default function TeamPage() {
  ...
  useGSAP(
    () => {
       ...
       * 3. TITLE LINE REVEAL
       * -----------------------------------------------------
       * Khi scroll tới section title:
       * - line đỏ xuất hiện từ trái sang phải
       * - chữ title hiện ra
       * - line thu lại và biến mất
       *
       * Bản này dùng scaleX thay vì translateX để line không bị hiện sẵn.
       */
      const titleTimeline = gsap.timeline({
        scrollTrigger: {
          trigger: SELECTORS.titleSection,
          start: "top 70%",
      ...
      titleTimeline
        // Trạng thái ban đầu
        .set([SELECTORS.titleBlock, SELECTORS.titleSubtitleBlock], {
          scaleX: 0,
          autoAlpha: 1,
          transformOrigin: "left center",
          force3D: true,
        })
        .set([SELECTORS.titleText, SELECTORS.titleSubtitleText], {
          autoAlpha: 0,
          y: 8,
          filter: "blur(2px)",
        })

        // 1. Cả 2 block đỏ cùng chạy từ trái sang phải
        .to([SELECTORS.titleBlock, SELECTORS.titleSubtitleBlock], {
          scaleX: 1,
          duration: 0.85,
          stagger: 0.12,
          ease: "expo.inOut",
        ...
        // 2. Cả title và text nhỏ cùng hiện
        .to(
          [SELECTORS.titleText, SELECTORS.titleSubtitleText],
          {
            autoAlpha: 1,
            y: 0,
            filter: "blur(0px)",
        ...
        // 3. Đổi điểm rút block sang bên phải
        .set([SELECTORS.titleBlock, SELECTORS.titleSubtitleBlock], {
          transformOrigin: "right center",
        })

        // 4. Cả 2 block cùng rút sang phải
        .to([SELECTORS.titleBlock, SELECTORS.titleSubtitleBlock], {
          scaleX: 0,
          duration: 0.85,
          ease: "expo.inOut",
  ...
  return (
    <main ref={pageRef} className="relative overflow-x-hidden">
      ...
      ===================================================== */}
      {/* PAGE 2A - TITLE */}
      <section className="team-title-section relative z-20 h-[50vh] overflow-hidden bg-white">
        <div
        ...
        <div
          ...
          style={{ color: teamPageData.teamColor }}
        >
          {teamPageData.titleLabelRight}
        </div>

        <div className="absolute left-1/2 top-[52%] z-50 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
          {/* TITLE BLOCK REVEAL */}
          <div className="relative w-fit overflow-hidden px-[0.8vw]">
            <div
              className="team-title-block absolute left-0 top-0 z-20 h-full w-full origin-left scale-x-0 will-change-transform"
              style={{ backgroundColor: teamPageData.teamColor }}
            />

            <h2
              className="team-title-text relative z-10 whitespace-nowrap font-akira text-[clamp(72px,13vw,190px)] uppercase leading-none tracking-[0.01em] scale-y-[1.08] opacity-0 will-change-[opacity,transform,filter]"
              style={{ color: teamPageData.teamColor }}
            >
              {teamPageData.titleText}
            </h2>
          </div>

          {/* SUBTITLE BLOCK REVEAL */}
          <div className="relative mt-5 w-fit overflow-hidden px-2">
            <div
              className="team-title-subtitle-block absolute left-0 top-0 z-20 h-full w-full origin-left scale-x-0 will-change-transform"
              style={{ backgroundColor: teamPageData.teamColor }}
            />

            <p
              className="team-title-subtitle-text relative z-10 whitespace-nowrap text-center font-akira text-[11px] uppercase leading-[1.6] tracking-[0.24em] opacity-0 will-change-[opacity,transform,filter]"
              style={{ color: teamPageData.teamColor }}
            >
              {teamPageData.titleSubtitle}
            </p>
          </div>
        </div>
      </section>
