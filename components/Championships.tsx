...
interface Driver { pos: number; name: string; team: string; points: number; wins: number; }
interface Constructor { pos: number; team: string; points: number; }
interface Stats { worldChampion: string; constructorChampion: string; totalRaces: number; mostWinsDriver: { name: string; wins: number; }; }
...
export default function Championships() {
  ...
  const [hoveredTeam, setHoveredTeam] = useState<{ name: string, x: number, y: number } | null>(null);
  const [hoveredDriver, setHoveredDriver] = useState<{ name: string, x: number, y: number } | null>(null);
  ...
  const getTeamLogo = (teamName: string) => {
    ...
    return `/logos/${map[teamName] || "F1.svg"}`;
  };
  ...
  useGSAP(() => {
    ...
    const applyBlockReveal = (block: any, content: any, tlObj: any, position?: string) => {
      tlObj.set(block, { transformOrigin: "left" }, position)
        .to(block, { scaleX: 1, duration: 0.4, ease: "power4.inOut" }, position)
    ...
    tlHeader.to([line1Ref.current, line2Ref.current], { scaleX: 1, duration: 0.5, ease: "power3.out", stagger: 0.1 }, "-=0.2")
      .to(boxRef.current, { opacity: 1, scale: 1, duration: 0.3, ease: "back.out(1.7)" }, "-=0.3");
    ...
    // Drivers
    gsap.fromTo(driversRef.current, { y: 30, opacity: 0 }, { 
    ...
    if (driversRef.current) {
      const rows = gsap.utils.toArray(driversRef.current.querySelectorAll('.row-reveal-container')) as HTMLElement[];
      rows.forEach((row) => {
    ...
    // Constructors
    gsap.fromTo(constructorsRef.current, { y: 30, opacity: 0 }, { 
      y: 0, opacity: 1, duration: 0.6,
      scrollTrigger: {
    ...
    if (constructorsRef.current) {
      const rows = gsap.utils.toArray(constructorsRef.current.querySelectorAll('.row-reveal-container')) as HTMLElement[];
      rows.forEach((row) => {
        const block = row.querySelector('.row-block');
        const content = row.querySelector('.row-content');
        if(block && content) {
    ...
    // Races
    gsap.fromTo(racesRef.current, { y: 30, opacity: 0 }, { 
      y: 0, opacity: 1, duration: 0.6,
      scrollTrigger: {
    ...
    gsap.fromTo('.race-card', { x: 30, opacity: 0 }, { 
  ...
  useEffect(() => {
    ...
    // Use passive: false to allow preventDefault to stop page scrolling
  ...
  if (!seasonData) return <div className="min-h-screen bg-transparent text-white pt-32 text-center">Data not found.</div>;
    ...
    <div id="championships" ref={mainContainerRef} className="page-content pt-[120px] px-6 md:px-[50px] pb-[80px] min-h-screen bg-transparent text-white overflow-x-hidden font-inter">
      ...
      <div className="max-w-[1400px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-4">
            <div className="relative inline-block w-fit overflow-hidden">
              <div ref={titleRef} className="relative opacity-0">
                ...
                <h1 className="relative page-title font-akira text-[2.5rem] md:text-[4rem] text-white uppercase tracking-[2px] leading-tight z-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  CHAMPIONSHIPS
                </h1>
              </div>
              {/* Block Reveal Div */}
              <div ref={titleBlockRef} className="absolute top-0 left-0 w-full h-full bg-[#e10600] origin-left scale-x-0 z-20"></div>
            </div>
            
            <div className="mt-4 md:mt-0 md:mb-2 z-20">
              <select className="bg-white/5 backdrop-blur-md text-white border border-white/10 rounded-md px-5 py-2.5 font-orbitron focus:outline-none focus:border-[#e10600] transition-colors cursor-pointer text-sm tracking-wider uppercase hover:bg-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.2)]">
                <option value="2020" className="bg-[#0a0a0a]">Season 2020</option>
              </select>
            </div>
          </div>
          
          <div className="relative w-full h-[24px]">
            {/* F1 Style Decorative Lines */}
            <div className="absolute bottom-0 left-0 w-[calc(100%-15px)] h-[18px] skew-x-[-30deg] origin-bottom-right">
              <div ref={line1Ref} className="absolute bottom-0 left-0 w-[calc(100%-10px)] h-[8px] bg-[#e10600] origin-left rounded-l-[1px] shadow-[0_0_12px_rgba(225,6,0,0.5)] scale-x-0 z-0"></div>
              <div ref={line2Ref} className="absolute top-0 left-0 w-[calc(100%-10px)] h-[8px] bg-[#e10600] origin-left rounded-l-[1px] shadow-[0_0_12px_rgba(225,6,0,0.5)] scale-x-0 z-0"></div>
              <div ref={boxRef} className="absolute bottom-0 right-0 w-[8px] h-[18px] bg-[#e10600] origin-bottom-right shadow-[0_0_15px_rgba(225,6,0,0.6)] rounded-r-[1px] opacity-0 scale-0"></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16 mb-16">
          {/* DRIVER STANDINGS */}
          <div ref={driversRef} className="opacity-0">
            <h2 className="font-orbitron text-2xl md:text-3xl text-white mb-6 uppercase border-l-4 border-[#e10600] pl-4 flex items-center drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
              Driver Standings
            </h2>
            <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.1)]">
              ...
              <div className="flex flex-col">
                {seasonData.drivers.map((driver) => (
                  <div key={driver.name} className="row-reveal-container relative">
                    <div className="row-block absolute inset-0 bg-[#e10600] origin-left scale-x-0 z-10"></div>
                    <div 
                      className="row-content opacity-0 flex items-center p-4 border-b border-white/5 hover:bg-white/10 transition-colors group relative cursor-default bg-transparent"
                      onMouseMove={(e) => handleDriverMouseMove(e, driver.name)}
                      onMouseLeave={handleDriverMouseLeave}
                    >
                      <div className="absolute left-0 top-0 w-1 h-full bg-[#e10600] scale-y-0 group-hover:scale-y-100 origin-bottom transition-transform duration-300 shadow-[0_0_10px_#e10600]"></div>
                      <div className={`w-12 md:w-16 text-center font-orbitron font-bold text-lg md:text-xl transition-colors
                          ${driver.pos === 1 ? 'text-[#D4AF37] drop-shadow-[0_0_8px_rgba(212,175,55,0.6)]' : 
                            driver.pos === 2 ? 'text-[#C0C0C0]' : 
                            driver.pos === 3 ? 'text-[#CD7F32]' : 'text-white/40 group-hover:text-white/80'}`}>
                        {driver.pos}
                      </div>
                      ...
                      <div className="w-20 md:w-24 text-right font-orbitron font-bold text-lg md:text-xl text-[#e10600] group-hover:drop-shadow-[0_0_8px_rgba(225,6,0,0.5)] transition-all">
                        {driver.points}
                      </div>
                    </div>
                  </div>
          ...
          <div ref={constructorsRef} className="opacity-0">
            <h2 className="font-orbitron text-2xl md:text-3xl text-white mb-6 uppercase border-l-4 border-[#e10600] pl-4 flex items-center drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
              Constructor Standings
            </h2>
            <div className="bg-white/5 backdrop-blur-2xl border border-white/10 rounded-xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.1)]">
              ...
              <div className="flex flex-col">
                {seasonData.constructors.map((team) => (
                  <div key={team.team} className="row-reveal-container relative">
                    <div className="row-block absolute inset-0 bg-[#e10600] origin-left scale-x-0 z-10"></div>
                    <div 
                      className="row-content opacity-0 flex items-center p-4 border-b border-white/5 hover:bg-white/10 transition-colors group relative cursor-default bg-transparent"
                    ...
                    >
                      <div className="absolute left-0 top-0 w-1 h-full bg-[#e10600] scale-y-0 group-hover:scale-y-100 origin-bottom transition-transform duration-300 shadow-[0_0_10px_#e10600]"></div>
                      <div className={`w-12 md:w-16 text-center font-orbitron font-bold text-lg md:text-xl transition-colors
                          ${team.pos === 1 ? 'text-[#D4AF37] drop-shadow-[0_0_8px_rgba(212,175,55,0.6)]' : 'text-white/40 group-hover:text-white/80'}`}>
                        {team.pos}
                      </div>
                      <div className={`flex-1 font-bold uppercase tracking-wide transition-colors text-sm md:text-base
                          ${team.pos === 1 ? 'text-[#D4AF37]' : 'text-white group-hover:text-[#e10600]'}`}>
                        {team.team}
                      </div>
                      <div className="w-20 md:w-24 text-right font-orbitron font-bold text-lg md:text-xl text-[#e10600] group-hover:drop-shadow-[0_0_8px_rgba(225,6,0,0.5)] transition-all">
                        {team.points}
        ...
        <div ref={racesRef} className="opacity-0 mb-10">
          <h2 className="font-orbitron text-2xl md:text-3xl text-white mb-6 uppercase border-l-4 border-[#e10600] pl-4 drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
            Race Calendar Results
          </h2>
          {/* Scrollable Container with Custom Mouse Wheel Event */}
          <div 
            ref={scrollContainerRef}
            className="flex overflow-x-auto pb-6 pt-2 px-2 -mx-2 gap-5 hide-scrollbar"
          >
            {seasonData.races.map((race) => (
              <div key={race.round} className="race-card opacity-0 min-w-[260px] md:min-w-[300px] bg-white/5 backdrop-blur-2xl border border-white/10 rounded-xl p-6 hover:border-[#e10600]/60 hover:bg-white/10 transition-all duration-300 group relative shadow-[0_8px_32px_rgba(0,0,0,0.2),inset_0_1px_1px_rgba(255,255,255,0.1)] hover:-translate-y-1 hover:scale-[1.02] hover:shadow-[0_15px_30px_rgba(225,6,0,0.15)]">
                <div className="absolute top-0 right-6 w-[2px] h-4 bg-[#e10600] shadow-[0_0_8px_#e10600]"></div>
                <div className="text-[#e10600] font-orbitron text-xs uppercase tracking-widest mb-2 font-bold drop-shadow-[0_0_5px_rgba(225,6,0,0.4)]">
                  Round {race.round}
                </div>
                <div className="text-white font-bold text-lg md:text-xl leading-snug mb-5 h-[56px] group-hover:text-[#e10600] transition-colors drop-shadow-[0_2px_5px_rgba(0,0,0,0.5)]">
                  {race.name}
                </div>
                <div className="pt-4 border-t border-white/10 relative">
                   <div className="absolute top-[-1px] left-0 w-10 h-[1px] bg-[#e10600] scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 shadow-[0_0_8px_#e10600]"></div>
                   <div className="text-white/50 text-xs uppercase tracking-wider mb-1 group-hover:text-white/70 transition-colors">Winner</div>
                   <div className="text-white font-orbitron text-sm md:text-base group-hover:drop-shadow-[0_0_5px_rgba(255,255,255,0.5)] transition-all">{race.winner}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Hover Team Logo Popup */}
      {hoveredTeam && (
        <div 
          className="fixed pointer-events-none z-[100] bg-gradient-to-br from-[#ffffff] to-[#e0e0e0] backdrop-blur-3xl rounded-xl p-3 border border-[#e10600]/50 shadow-[0_15px_40px_rgba(225,6,0,0.2),inset_0_1px_3px_rgba(255,255,255,1)] flex flex-col items-center justify-center transform -translate-y-1/2 transition-opacity duration-200"
          style={{ 
            left: hoveredTeam.x + 20 > (typeof window !== 'undefined' ? window.innerWidth - 220 : 0) ? hoveredTeam.x - 240 : hoveredTeam.x + 20,
            top: hoveredTeam.y
          }}
        >
          {/* F1 Decorative Corners */}
          ...
          <div className="w-36 h-24 md:w-44 md:h-28 flex items-center justify-center px-4">
            <img 
              ...
              className="max-w-full max-h-full object-contain drop-shadow-[0_2px_6px_rgba(0,0,0,0.4)]" 
            />
          </div>
        </div>
      )}

      {/* Hover Driver Image Popup */}
      {hoveredDriver && (
        <div 
          className="fixed pointer-events-none z-[100] bg-gradient-to-br from-[#111] to-[#222] backdrop-blur-3xl rounded-xl p-3 border border-[#e10600]/40 shadow-[0_15px_40px_rgba(225,6,0,0.3),inset_0_1px_1px_rgba(255,255,255,0.1)] flex flex-col items-center justify-center transform -translate-y-1/2 transition-opacity duration-200"
          style={{ 
            left: hoveredDriver.x + 20 > (typeof window !== 'undefined' ? window.innerWidth - 250 : 0) ? hoveredDriver.x - 270 : hoveredDriver.x + 20,
            top: hoveredDriver.y
          }}
        >
          {/* F1 Decorative Corners */}
          ...
          <div className="w-40 h-40 md:w-48 md:h-48 rounded-lg overflow-hidden border border-white/10 bg-[#0a0a0a] shadow-inner flex items-center justify-center">
             <img 
               ...
               className="w-full h-full object-cover object-top filter contrast-125 saturate-110 drop-shadow-[0_0_10px_rgba(255,255,255,0.1)]" 
             />
          </div>
          <div className="mt-3 text-white font-orbitron text-sm font-bold uppercase tracking-wider text-center drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
            {hoveredDriver.name}
          </div>
        </div>
      )}
